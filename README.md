...
- Korean (Main Target)
- French (Prototype, Under Teseting)
- Support for the remaining languages ​​will be added later, after testing for the two languages ​​is completed. (due to a Checking for expected issues with some languages that mixed with alphabets as French/Português/etc, Toolbox refactoring and etcs)

Compatible Mods (May not always be guaranteed):
- Expanded Storage by jakiepoo <https://modworkshop.net/mod/56122>
- ImmersiveXP from "Oldman's Immersive Overhaul" (Mods Compilation) <https://modworkshop.net/mod/50811>
- Trader Refresh Hotkey from "Temporary fix by metro" <https://modworkshop.net/mod/55933>
...
| Tool                                  | Role                                                                                                              |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `unpack_and_decompile_pck.bat`      | Local dev helper — invokes `gdre_tools.exe` to decompile a locally-owned PCK (Windows, private workspace only) |
| `parse_translatables.py`            | Run all three parsers below sequentially                                                                          |
...
| `utils/translate_with_deepl.py`     | DeepL API caller with placeholder protection + XML-escape (used by orchestrator)                                  |
...
| `utils/build_runtime_tsv.py`        | Build runtime TSV from xlsx (called by `build_mod_package`)                                                     |
| `utils/build_attributions.py`       | Generate `Texture_Attribution.md` from `Texture.xlsx` (called by `build_mod_package`)                       |
| `utils/build_translation_credit.py` | Generate `Translation_Credit.md` per locale (called by `build_mod_package`)                                   |
| `utils/build_authors.py`            | Update the auto-generated Translators section of `AUTHORS.md` (called by `build_mod_package`)                 |
...
| 도구                                  | 역할                                                                                                                   |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `unpack_and_decompile_pck.bat`      | 로컬 개발 편의 스크립트 —`gdre_tools.exe` 를 호출해 본인이 소유한 PCK 를 디컴파일 (Windows, private 작업 공간 전용) |
| `parse_translatables.py`            | 아래 3개 파서를 순차 실행                                                                                              |
...
| `utils/translate_with_deepl.py`     | DeepL API 호출 도구 (placeholder 보호 + XML escape; orchestrator가 호출)                                               |
...
| `utils/build_runtime_tsv.py`        | xlsx → 런타임 TSV 빌드 (`build_mod_package`가 호출)                                                                 |
| `utils/build_attributions.py`       | `Texture.xlsx` → `Texture_Attribution.md` 생성 (`build_mod_package`가 호출)                                     |
| `utils/build_translation_credit.py` | 로케일별 `Translation_Credit.md` 생성 (`build_mod_package`가 호출)                                                 |
| `utils/build_authors.py`            | `AUTHORS.md` 의 자동 생성 Translators 섹션 갱신 (`build_mod_package`가 호출)                                       |
