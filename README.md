...
| `parse_tscn_text.py`          | Parse `.tscn` scene files for translatable text         |
| `parse_tres_text.py`          | Parse `.tres` resource files for translatable text      |
| `parse_gd_text.py`            | Parse `.gd` scripts for UI strings                      |
...
| `parse_tscn_text.py`          | `.tscn` 씬 파일을 파싱해 번역 대상 텍스트 추출          |
| `parse_tres_text.py`          | `.tres` 리소스 파일을 파싱해 번역 대상 텍스트 추출      |
| `parse_gd_text.py`            | `.gd` 스크립트를 파싱해 UI 문자열 추출                  |
