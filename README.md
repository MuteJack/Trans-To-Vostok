...
Translated texture assets (images) are made from a mix of hand-crafted work, license-free assets, and third-party data sources. Per-file source credits are listed in **`Trans To Vostok/<locale>/Texture_Attribution.md`** inside the mod zip — auto-generated from the translation workbook each build.
...
번역된 텍스처(이미지) 에셋은 직접 작업물 / 라이선스-프리 애셋 / 제3자 데이터 출처가 혼합되어 있습니다. 각 파일별 출처는 모드 zip 안의 **`Trans To Vostok/<locale>/Texture_Attribution.md`** 에 정리되어 있으며, 빌드 시 번역 워크북에서 자동 생성됩니다.
