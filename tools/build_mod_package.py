...
def build_attributions_for_locale(tools_dir: Path, locale: str) -> bool:
    """Call build_attributions.py. Skipped if Texture.xlsx is absent.
    Output goes to the default path of build_attributions.py (<pkg_root>/<locale>/Texture_Attribution.md)."""
...
def build_texture_meta_for_locale(tools_dir: Path, locale: str) -> bool:
    """Call build_texture_meta.py. Emits Trans To Vostok/<locale>/texture_meta.json
    mapping each translated texture rel_path to its method ('replace' or 'blend'),
    consumed at runtime by texture_loader.gd."""
    print(f"=== Building texture_meta.json: {locale} ===")
    cmd = [sys.executable, "utils/build_texture_meta.py", locale]
    result = subprocess.run(cmd, cwd=tools_dir)
    if result.returncode != 0:
        print(f"[ERROR] {locale} texture_meta build failed")
        return False
    print()
    return True
...
def build_translation_credit_for_locale(tools_dir: Path, locale: str) -> bool:
    ...
    result = subprocess.run(cmd, cwd=tools_dir)
    if result.returncode != 0:
        print(f"[ERROR] {locale} Translation_Credit build failed")
        return False
    print()
    return True


def build_authors_md(tools_dir: Path) -> bool:
    """Call build_authors.py. Updates the auto-generated Translators section
    of AUTHORS.md (project root). Project-wide; not per locale."""
    print(f"=== Building AUTHORS.md ===")
    cmd = [sys.executable, "utils/build_authors.py"]
    result = subprocess.run(cmd, cwd=tools_dir)
    if result.returncode != 0:
    ...
    try:
        with zipfile.ZipFile(tmp_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for locale in locales:
                ...
                # 5. Texture_Attribution.md → Trans To Vostok/<locale>/Texture_Attribution.md
                # generated by build_attributions.py. Locales without Texture.xlsx have no file → skip
                attribution_path = locale_dir / "Texture_Attribution.md"
                if attribution_path.exists() and attribution_path.is_file():
                    zf.write(attribution_path, f"{MOD_NAME}/{locale}/Texture_Attribution.md")
                    count += 1

                # 6. Translation_Credit.md → Trans To Vostok/<locale>/Translation_Credit.md
                # generated by build_translation_credit.py
                credit_path = locale_dir / "Translation_Credit.md"
                if credit_path.exists() and credit_path.is_file():
                ...
                # 8. texture_meta.json → Trans To Vostok/<locale>/texture_meta.json
                # consumed by texture_loader.gd to route each texture to
                # 'replace' (substitute as-is) or 'blend' (Image.blend_rect on
                # top of original). Generated by build_texture_meta.py from
                # Texture canonical TSVs.
                texture_meta = locale_dir / "texture_meta.json"
                if texture_meta.exists() and texture_meta.is_file():
                    zf.write(texture_meta, f"{MOD_NAME}/{locale}/texture_meta.json")
                    count += 1
...
def main() -> int:
    ...
    # 2. build attributions (only for locales with Texture.xlsx)
    for locale in locales:
    ...
    for locale in locales:
        ...
        if not build_translation_credit_for_locale(script_dir, locale):
            return 1
        if not build_texture_meta_for_locale(script_dir, locale):
            return 1
