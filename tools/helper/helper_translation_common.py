"""Shared validation helpers for Translation.xlsx.

Utility module — not directly callable. Exposes the helpers used by:
  - tools/build/build_runtime_tsv.py
  - tools/validation/check_deprecated.py
  - tools/validation/check_missing.py
  - tools/validation/check_whitespace_text.py

Exports (high level):
  validate_xlsx(xlsx_path, tsv_dir, soft) -> ValidationResult
      Full validation entry; returns ValidationResult with error/warning
      counts and log path.
  load_all_translation_sheets(xlsx_path)
      Read every non-MetaData sheet as (sheet_name, header, rows).
  load_metadata(xlsx_path)
      Read the MetaData sheet into a dict.
  _effective_method(row)
      Default empty method to 'literal'; pass 'ignore' through verbatim.
  load_tsv_index(tsv_dir) / load_tres_text_set(tsv_dir) /
  load_gd_text_set(tsv_dir)
      Build lookup indexes from .tmp/parsed_text/ for cross-checks.

Schema and check semantics:
