"""Convert Translation.xlsx to canonical TSVs for one locale.

xlsx -> TSV side of the editing/VCS workflow, restricted to the
Translation category. Companion file build_texture_tsv.py handles
Texture.xlsx symmetrically.

Path mapping:
    Translations/<locale>/Translation.xlsx
        ->
    Translations/<locale>/tsv/Translation/<sheet>.tsv
...
Behavior:
    ...
    - `_sheet_order.txt` written alongside the per-sheet TSVs so
      rebuild_translation_xlsx.py can preserve sheet order.

Input convention:
    <locale>   single locale
    Template   Template's Translation.xlsx
    all        every locale with enabled=true (English/Template excluded)

Usage:
    python tools/translation/build_translation_tsv.py Korean
    python tools/translation/build_translation_tsv.py Template
    python tools/translation/build_translation_tsv.py all
"""
import argparse
import csv
import sys
...
def main(argv: list[str]) -> int:
    ...
    _say(f"=== build_translation_tsv ({len(locales)} target(s)) ===")
